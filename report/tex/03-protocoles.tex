...
Le protocole s'appuie sur une clé connue (appelée passphrase) par les \acrshort{sta} et l'\acrshort{ap}, nommée \acrfull{psk}. Il peut également, dans le cadre de \textit{WPA-Enterprise}, être généré via un serveur RADIUS.
...
\begin{itemize}
    ...
    \\Les nonces (\Gls{anonce}, générée par l'\acrshort{ap} et \Gls{snonce}, générée par la \acrshort{sta}) garantissent l'unicité de la PTK à chaque association, même si la PMK reste identique. Le \acrshort{mic}, calculé à l'aide de la clé \acrfull{kck}, assure l'intégrité et l'authenticité des messages du handshake.
    ...
    \begin{itemize}
        ...
        \item \textbf{PTK (Pairwise Transient Key, 512 bits)} : clé unique unicast \acrshort{sta} $\leftrightarrow$ \acrshort{ap}. Utilise une \acrfull{prf}, pour faire cette dérivation : 
        \[
            \begin{aligned}
    ...
    \item \textbf{Phase 3 (TKIP par trame)} : une fois les clés installées, chaque trame de données est chiffrée indépendamment à l’aide de l’algorithme \acrshort{tkip}.
    Contrairement à WEP, WPA ne réutilise jamais une même clé de chiffrement pour plusieurs trames : une clé différente est dérivée pour chaque paquet transmis.

    Pour chaque trame, les éléments suivants sont utilisés :
    \begin{itemize}
        \item la clé temporaire \textbf{TK} (Temporal Key), issue du \acrshort{ptk} ;
        \item l’adresse MAC de l’émetteur, utilisée pour lier la clé à la station ;
        \item le \acrfull{tsc} (TKIP Sequence Counter), un compteur de 48 bits jouant le rôle d’IV étendu et de mécanisme anti-rejeu.
    \end{itemize}

    \medskip
    TKIP applique un \textbf{mélange de clés en deux phases} afin de produire une clé RC4 unique pour chaque trame :

    \begin{itemize}
        \item \textbf{Phase 1 (Key Mixing Phase 1)} : la clé TK, l’adresse MAC source et les 32 bits de poids fort du TSC sont combinés pour produire une clé intermédiaire. Cette étape permet de limiter la réutilisation des IV ;
        \item \textbf{Phase 2 (Key Mixing Phase 2)} : les 16 bits de poids faible du TSC sont ensuite intégrés à la clé intermédiaire afin de générer la clé finale spécifique au paquet, appelée \acrshort{ppk}, qui servira d’entrée à l’algorithme RC4. Il y aura également l'utilisation de 16 bits de l'\acrshort{iv} ainsi que des bits spéciaux \textit{d}, utilisés pour éviter d'avoir des \acrshort{iv} faibles.
    \end{itemize}

    \medskip
    Avant le chiffrement, un \acrfull{mic} est calculé sur les données en clair à l’aide de l’algorithme \textbf{Michael}. Ce MIC protège le contenu contre les modifications actives et est concaténé aux données avant le calcul de l’ICV (CRC-32).

    L’ensemble \textit{(données || MIC || ICV)} est ensuite chiffré par XOR avec le flot de clés généré par RC4, comme illustré par les figures \ref{fig:wpa_tkip_frame_1} à \ref{fig:wpa_tkip_frame_2}.

    On obtient finalement le paquet \ref{fig:wpa_tkip_full}, suite au XOR effectué sur les algorithmes.

    Pour rappel, l'\acrshort{iv} est composé du \acrshort{tsc}, ce qui signifie qu'à chaque échange, celui-ci va être incrémenter. Et par conséquent, l'\acrshort{iv} sera à son tour unique.
...
\begin{figure}[H]
    ...
    \node[box, right=1.2cm of iv] (ptk) {PTK \\ (128 bits)};
