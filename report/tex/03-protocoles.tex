...
\begin{itemize}
    ...
    \item un \acrfull{mic} pour de l'intégrité ;
...
\begin{itemize}
    \item \textbf{Phase 1 (\acrshort{pbkdf2} 4096 itérations)} : passphrase $\rightarrow$ PMK résistante dictionnaire (impossible bruteforce hors ligne contrairement à WEP).
    
    \item \textbf{Phase 2 (4-Way \Gls{handshake})} : \acrshort{pmk} $\rightarrow$ \acrshort{ptk} échangée sans jamais transmettre la clé (\Gls{anonce}/\Gls{snonce} + \acrshort{mic} protègent). Clés éphémères.
    
    \item \textbf{Phase 3 (\acrshort{tkip} par trame)} : 
    \begin{itemize}
        \item TSC 48 bits (vs IV 24b WEP)
        \item MIC (Michael) $\rightarrow$ intégrité (vs CRC-32 faible)
        \item RC4 + TSC $\rightarrow$ keystream unique par trame
    \end{itemize}
\end{itemize}
...
\node[font=\small\bfseries, above=0.1cm of handshake] {Phase 2 : Création d'une clé unique en \acrshort{ap} et \acrshort{sta}};
...
\node[box, below=1.2cm of tk] (rc4) {RC4\\Keystream};
\node[box, below=1.2cm of tsc] (ct) {Ciphertext};
...
\begin{itemize}
    ...
    \item \textbf{PTK (Pairwise Transient Key, 512 bits)} : clé unique unicast \acrshort{sta} $\leftrightarrow$ \acrshort{ap}, dérivée de : $PTK = PRF(PMK, SSID, min(\Gls{anonce},\Gls{snonce}) || max(\Gls{anonce},\Gls{snonce}))$.
