...
aux réseaux Wi-Fi n’a cessé de croître, sous les effets de l’évolution des protocoles, de la
diversité des implémentations matérielles et logicielles, et de la généralisation des usages mobiles.\\

Contrairement aux réseaux filaires, les communications Wi-Fi reposent sur des ondes radios, 
accessible à tout appareil situé dans la zone de couverture du point d’accès. Cette caractéristique
 expose les échanges à des menaces spécifiques telles que l’écoute
...
attaques statistiques ayant compromis \acrshort{wep} ont laissé place à des attaques ciblant les mécanismes
d’authentification de \acrshort{wpa}, puis à des attaques protocolaires plus subtiles contre \acrshort{wpa2}, avant
...
sécurité déployés pour protéger les communications sans fil. Dès l’apparition du protocole \acrshort{wep}, 
introduit avec la première version du standard IEEE 802.11 en
...
\acrshort{wep} repose sur l’algorithme de chiffrement par flot \acrshort{rc4}, combiné à des \acrfull{iv} de seulement 24 bits. Cette taille réduite entraîne une réutilisation fréquente des \acrshort{iv} sur des
réseaux actifs, ouvrant la voie à des attaques statistiques permettant de récupérer progressivement
la clé de chiffrement. Ces attaques ont été rapidement automatisées par des
outils spécialisés, rendant la compromission d’un réseau \acrshort{wep} accessible avec des moyens limités.
Face à ces vulnérabilités structurelles, \acrshort{wep} a été officiellement déclaré obsolète au début des
années 2000.\\

Afin de répondre à l’urgence sécuritaire sans imposer un renouvellement massif des équipements,
la Wi-Fi Alliance a introduit \acrshort{wpa} comme solution transitoire.
\acrshort{wpa} s’appuie sur le protocole \acrshort{tkip}, qui conserve \acrshort{rc4}
mais introduit un mécanisme de renouvellement dynamique des clés, un compteur de séquence et
un code d’intégrité des messages. Bien que \acrshort{wpa} ait significativement élevé le niveau de sécurité
par rapport à \acrshort{wep}, plusieurs attaques ont montré que cette solution restait vulnérable, notamment
en raison de son héritage cryptographique et de certaines faiblesses dans la gestion de l’intégrité
des \glspl{trame}.\\

La normalisation de \acrshort{wpa2} en 2004 marque une étape
majeure dans la sécurisation des réseaux Wi-Fi. \acrshort{wpa2} abandonne \acrshort{rc4} au profit de l’algorithme
AES dans le mode CCMP, offrant une protection complète en matière de confidentialité,
d’intégrité et de protection contre les attaques par rejeu. Pendant plusieurs années, \acrshort{wpa2} a été
...
étapes du \gls{handshake} afin de forcer la réinstallation de clés de chiffrement déjà
utilisées.\\
...
constats, \acrshort{wpa3} a été introduit en 2018 avec des mécanismes d’authentification renforcés,
...
\acrshort{wpa2} et \acrshort{wpa3} dans des environnements mixtes maintient une surface d’attaque significative,
justifiant l’étude approfondie des attaques visant les protocoles encore largement déployés.

\subsection{Attaques par protocole}

\subsubsection{Attaques contre WEP : premières vulnérabilités des réseaux Wi-Fi}

Le protocole \acrshort{wep} a constitué la première tentative de
sécurisation des réseaux Wi-Fi lors de l’introduction du standard IEEE 802.11. Conçu pour offrir
un niveau de confidentialité comparable à celui des réseaux filaires, \acrshort{wep} repose sur
l’algorithme de chiffrement par flot \acrshort{rc4}, combiné à une clé secrète partagée et à un \acrfull{iv} de 24 bits transmis en clair dans chaque \gls{trame}. Cette conception s’est
rapidement révélée inadaptée face aux contraintes spécifiques des réseaux sans fil.\\

La principale faiblesse de \acrshort{wep} réside dans la gestion des vecteurs d’initialisation. En raison de
...
Les travaux de Fluhrer, Mantin et Shamir\cite{FluhrerM7:online} ont mis en évidence une vulnérabilité structurelle du
générateur de clés de \acrshort{rc4} lorsqu’il est utilisé avec des IV faiblement aléatoires. L’attaque dite
FMS exploite des corrélations entre certains octets du flot de clés et la clé secrète, permettant,
après l’analyse d’un volume suffisant de trafic, de reconstituer progressivement la clé \acrshort{wep}.
...
D’autres attaques ont rapidement émergé pour exploiter les faiblesses de \acrshort{wep}. Les attaques par
...
tronquer progressivement une \gls{trame} chiffrée et à observer les réponses du point d’accès afin de
déduire le contenu en clair octet par octet. Ces attaques démontrent que \acrshort{wep} ne garantit ni la
...
cassage de clés \acrshort{wep} accessible avec des moyens matériels limités et dans des délais très courts,
souvent de l’ordre de quelques minutes. Face à l’accumulation de ces vulnérabilités, \acrshort{wep} a été
officiellement déclaré obsolète, et son utilisation est aujourd’hui considérée comme totalement
non-sécurisée.\\

Bien que \acrshort{wep} ne soit plus déployé dans les environnements modernes, l’étude de ses attaques
...
\subsubsection{Attaques contre WPA et TKIP : limites des solutions transitoires}

Face à l’obsolescence rapide de \acrshort{wep}, la Wi-Fi Alliance a introduit en 2003 le protocole \acrshort{wpa}
comme mécanisme de sécurisation transitoire, dans l’attente de la
finalisation du standard IEEE 802.11i. L’objectif principal de \acrshort{wpa} était de corriger les faiblesses
les plus critiques de \acrshort{wep} tout en restant compatible avec les équipements existants, via une mise
à jour logicielle. Pour ce faire, \acrshort{wpa} repose sur le protocole \acrfull{tkip}, qui conserve l’algorithme \acrshort{rc4} mais en modifie profondément l’usage.\\

\acrshort{tkip} introduit plusieurs améliorations majeures par rapport à \acrshort{wep}. Il met en œuvre un
mécanisme de mélange de clés visant à produire une clé de chiffrement distincte pour chaque
\gls{trame}, à partir d’une clé maîtresse partagée et de paramètres dynamiques. Il ajoute également un
compteur de séquence étendu afin de prévenir les attaques par rejeu, ainsi qu’un code d’intégrité
des messages, appelé \acrfull{mic}, destiné à détecter les modifications
non autorisées des trames.\\

Malgré ces améliorations, \acrshort{wpa}/\acrshort{tkip} conserve des faiblesses structurelles héritées de \acrshort{wep}, en
particulier l’utilisation de \acrshort{rc4} et certaines limitations dans la conception du mécanisme
d’intégrité. Plusieurs attaques ont démontré que \acrshort{tkip} ne garantissait pas une sécurité
...
ChopChop avec l’exploitation des limites du \acrfull{mic}, rendant possible la falsification de trafic
chiffré en un temps relativement court.\\

Ces attaques ont mis en évidence le caractère fondamentalement transitoire de \acrshort{wpa}. Bien que
plus robuste que \acrshort{wep}, \acrshort{wpa}/\acrshort{tkip} ne constitue pas une solution pérenne, en raison de ses choix
cryptographiques contraints par la rétrocompatibilité matérielle. La découverte de vulnérabilités
exploitables a conduit à l’interdiction progressive de \acrshort{tkip} dans les déploiements modernes et à
son déclassement au profit de mécanismes reposant sur des algorithmes de chiffrement par bloc
plus robustes.\\

L’étude des attaques contre \acrshort{wpa} et \acrshort{tkip} demeure néanmoins pertinente, car elle illustre les
...
conception de \acrshort{wpa2} et des protocoles ultérieurs.

\subsubsection{Attaques contre WPA2 : exploitation du 4-Way Handshake}

La standardisation de \acrshort{wpa2} en 2004, dans le cadre du protocole IEEE 802.11i, a marqué une étape
majeure dans la sécurisation des réseaux Wi-Fi. En remplaçant \acrshort{rc4} par l’algorithme AES utilisé
dans le mode CCMP, \acrshort{wpa2} apporte une amélioration significative en matière de confidentialité,
d’intégrité et de protection contre les attaques par rejeu. \acrshort{wpa2} s’est ainsi imposé comme le
...
Le fonctionnement de \acrshort{wpa2} repose sur un échange en quatre étapes, appelé \gls{handshake}, destiné à établir des clés de session uniques entre le client et le point d’accès. À
partir d’une clé maîtresse partagée (\acrshort{pmk}), dérivée soit d’une \gls{passphrase} dans le mode
\textit{WPA2-Personal}, soit des paramètres d’authentification 802.1X dans le mode
\textit{WPA2-Enterprise}, le protocole permet de générer une clé temporaire de session (\acrshort{ptk})
...
\gls{handshake} lors de l’authentification d’un client légitime, puis à effectuer un
craquage hors ligne de la \gls{passphrase}. Cette attaque ne nécessite aucune interaction
...
remis en question la sécurité de \acrshort{wpa2}. Cette attaque exploite une faiblesse protocolaire du
\gls{handshake}, en forçant la retransmission de certains messages afin de provoquer la
...
\acrshort{wpa2} et a constitué un facteur déterminant dans le développement et l’adoption de \acrshort{wpa3}.\\

Malgré ces vulnérabilités, \acrshort{wpa2} demeure largement déployé dans de nombreux environnements,
en particulier dans les réseaux domestiques et les petites infrastructures professionnelles. Ce
constat justifie l’étude approfondie des attaques ciblant \acrshort{wpa2}, qui restent aujourd’hui parmi les
menaces les plus réalistes et les plus exploitables dans un contexte opérationnel.

\subsubsection{Focus sur WPA3 et les attaques Dragonblood}

\acrshort{wpa3} a été introduit afin de renforcer la sécurité des réseaux Wi-Fi, notamment en remplaçant le
...
\acrshort{wpa3} impose également l’usage des \textit{Protected Management Frames} (PMF), réduisant
...
publiée par Mathy Vanhoef et Eyal Ronen\cite{Dragonbl69:online}. Ces vulnérabilités ne remettent pas en cause le principe
...
accès mémoire, consommation de ressources) lors de cette dérivation.\\ 

Un attaquant capable
...
contraignantes que le craquage hors ligne classique de \acrshort{wpa2}, elles soulignent l’importance de
...
configuré pour accepter à la fois \acrshort{wpa2} et \acrshort{wpa3}. Dans ce
contexte, un attaquant peut tenter d’influencer la négociation afin de forcer l’usage de mécanismes
moins robustes (par exemple un retour vers \acrshort{wpa2}) ou de paramètres cryptographiques plus
...
L’existence de \textit{Dragonblood} montre ainsi que l’adoption de \acrshort{wpa3} ne supprime pas
...
\subsection{Autres types d'attaques}

\subsubsection{Attaques PMKID : compromission sans client connecté\cite{Bulletin6:online}}

Les attaques PMKID constituent une évolution significative des techniques de compromission des
réseaux \acrshort{wpa2}, en ce qu’elles permettent de lancer une attaque par dictionnaire sans nécessiter la
capture préalable du \gls{handshake} ni la présence d’un client déjà authentifié sur le
...
Le PMKID (\textit{Pairwise Master Key Identifier}) est une valeur dérivée de la clé maîtresse \acrshort{pmk}
et transmise par le point d’accès dans certaines trames EAPOL lors de l’initialisation d’une
connexion. Il est calculé à partir d’une fonction de hachage cryptographique incluant la \acrshort{pmk}, les
...
peut être transmise avant même l’établissement complet du \gls{handshake}.\\
...
manière similaire aux attaques basées sur le \gls{handshake}, mais sans nécessiter
...
Comme pour les attaques par capture du \gls{handshake}, la réussite de l’attaque PMKID
dépend essentiellement de la robustesse de la \gls{passphrase} utilisée. Les réseaux reposant sur
...
migration vers \acrshort{wpa3}. En remplaçant l’échange de clés par le protocole SAE, \acrshort{wpa3} supprime la
...
\subsubsection{Attaques par désauthentification et déni de service}
...
authentifiées, y compris lorsque les communications de données sont protégées par \acrshort{wep}, \acrshort{wpa} ou
\acrshort{wpa2}. Un attaquant situé à portée radio peut ainsi forger des trames de désauthentification en
...
de nombreuses autres attaques Wi-Fi. 

En forçant la déconnexion d’un client, l’attaquant provoque
une reconnexion automatique, générant ainsi un \gls{handshake} pouvant être capturé à
des fins de craquage hors ligne. Cette technique est largement utilisée pour accélérer les attaques
contre les réseaux \acrshort{wpa}/\acrshort{wpa2}-Personal.\\
...
passif d’identifier le SSID réel. 

De manière plus générale, la désauthentification facilite la mise
...
associés à un réseau donné. 

Cette facilité d’exploitation rend les attaques par
...
soit optionnel dans \acrshort{wpa2}, il devient obligatoire avec \acrshort{wpa3}, réduisant significativement
l’efficacité de ces attaques. 

Néanmoins, la persistance de réseaux n’implémentant pas PMF
maintient les attaques par désauthentification parmi les menaces les plus répandues dans les
environnements Wi-Fi actuels.

\subsubsection{Attaques Evil Twin et usurpation de points d’accès}
...
certains cas, son adresse MAC. 

L’attaquant configure généralement ce point d’accès malveillant
...
Dans le cas des réseaux ouverts ou protégés par \acrshort{wpa}/\acrshort{wpa2}-Personal, l’attaquant
...
du point d’accès. Même lorsque la \gls{passphrase} saisie est correcte, celle-ci peut être capturée
par l’attaquant et réutilisée ultérieurement pour se connecter au réseau légitime. 
...
Ces attaques combinées permettent de compromettre des identifiants
...
techniques et organisationnels. L’utilisation de \acrshort{wpa2}-Enterprise ou \acrshort{wpa3}-Enterprise avec
...
L’activation des \textit{Protected Management Frames} limite l’efficacité des
...
\subsubsection{Attaques par compromission applicative : Man-in-the-Middle et SSL Stripping}
...
reposant sur HTTP et HTTPS.

Contrairement aux attaques ciblant directement les mécanismes de
...
En position d’homme du milieu, l’attaquant intercepte cette redirection et
...
lorsque les mécanismes de validation côté client sont insuffisants. 

Dans un environnement Wi-Fi
...
downgrade vers HTTP et neutralise efficacement les attaques de type SSL Stripping. 
