...
Dans WPA utilisant TKIP, le champ historiquement appelé \textbf{IV} ne correspond plus à un vecteur d’initialisation aléatoire comme en WEP.  
Il transporte en réalité une représentation structurée du \acrfull{tsc}, d’où le terme d’\textit{IV étendu}.

Le TSC est un compteur de 48 bits, unique et incrémenté pour chaque trame émise. Il est utilisé pour générer une clé de chiffrement différente pour chaque paquet, ce qui empêche les attaques par rejouage et améliore la sécurité du protocole.  

\paragraph{Composition de l'IV étendu (16 bits dans la trame) :}  
...
\begin{itemize}
    ...
    \item \textbf{Ext. IV / Reserved (6 bits) :} Ce champ complète le TSC en incluant les bits de poids fort (pour former les 48 bits du compteur de séquence) et peut également contenir des indicateurs de fragmentation ou des bits réservés pour la compatibilité. Il garantit l’unicité de la clé de chiffrement pour chaque trame et permet de gérer correctement les trames fragmentées.
\end{itemize}
...
Comme les bits de poids fort du TSC évoluent lentement, cette phase limite efficacement les collisions de clés tout en restant peu coûteuse en calcul.
...
\begin{itemize}
    ...
    \item des \textbf{bits de contrôle de l’IV}, notamment le bit \textit{ExtIV} et des bits destinés à éviter les IV faibles connus de RC4.
\end{itemize}

Cette phase produit la \textbf{clé finale par trame}, appelée \acrshort{ppk}, utilisée comme entrée de l’algorithme RC4.  
Ainsi, même deux trames consécutives émises par une même station ne partageront jamais la même clé de chiffrement.

\medskip
Avant le chiffrement, un \acrfull{mic} est calculé sur les données en clair à l’aide de l’algorithme \textbf{Michael}.  
...
\medskip
L’ensemble \textit{(données || MIC || ICV)} est finalement chiffré par un XOR avec le flot de clés généré par RC4, produisant la trame WPA-TKIP transmise sur le médium radio.
